    ...
    return (
      <div className='blockUser row'>
        ...
        <div className='row rowElse '>
            ...
            : <div className="elemBlock" >{this.state.first_name}</div> } 

            {(this.state.onEdit) ?
            <input type='text' name='last_name' onChange={this.onChange} value={this.state.last_name}/>
            : <div className="elemBlock">{this.state.last_name}</div> }
            
            <div className='w-100'/>

            {(this.state.onEdit) ?
            <input type='text' name='email' onChange={this.onChange} value={this.state.email}/>
            : <div className="elemBlock">{this.state.email}</div> }
        </div>
        
        <div className='ml-auto'>
        {(this.state.onEdit) ? 
            <button className='btn btn-info mb-4 mt-31 btnElse ' onClick={this.saveUserInfo}>Сохранить</button> :
            <div> 
            <button className='btn btn-info mb-4 mt-31 btnElse' onClick={() => this.setState({onEdit: true})} >Редактировать</button>
            <div className='w-100'/>
